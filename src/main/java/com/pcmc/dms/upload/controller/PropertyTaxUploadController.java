...
import com.pcmc.dms.upload.service.ProptaxService;

@Controller
public class PropertyTaxUploadController {
	
	@Autowired
	private ProptaxService propTaxService;
	
	private static final String UPLOAD_URL = "uploadDataPropTax";
	...
	public String upload(@RequestParam CommonsMultipartFile file, HttpSession session, final RedirectAttributes redirectAttributes) {
		...
		try {
			...
			propTaxService.readBooksFromExcelFile(filePath);
	...
	@RequestMapping(value ="/downloadPropTaxTemplate", method = RequestMethod.GET)
	public ModelAndView downloadTemplate() {
		
		ModelAndView modelAndView = new ModelAndView("propTaxTemplate", "obj", null);
		return modelAndView;
	}
