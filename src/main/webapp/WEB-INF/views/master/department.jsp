	...
	<form:form id="some-form" commandName="model" class="form-horizontal"
		...
		<div class="form-group">
			...
			<div class="col-sm-3">
				...
				<form:input path="deptName" class="col-sm-5 form-control" />
			</div>
		</div>
				...
				<tbody>
					<c:forEach items="${ masterList }" var="item">
						<tr>
							<td>${ item.deptId  }</td>
							<td>${ item.deptName  }</td>
							<td><a href="./edit${ postUrl }?id=${ item.deptId }">Edit</a></td>
							<td><a href="./delete${ postUrl }?id=${ item.deptId }">Delete</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
